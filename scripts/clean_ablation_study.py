...
def run_simulation(bars, mode="ON", context_bars=None):
    ...
    # Intercept regime detector cleanly without duplicating math
    real_detect = app.analysis.regime.detect_market_regime
    real_calc_vol = app.analysis.regime.calculate_volatility
    
    def mock_detect(*args, **kwargs):
        # 1. Call real detector to see what production would do
        ...
        if mode == "OFF" and res_real.regime == "HIGH_VOLATILITY":
            # 2. To get the exact underlying directional regime without duplicating math,
            # we temporarily zero out volatility so it falls through to the trend logic.
